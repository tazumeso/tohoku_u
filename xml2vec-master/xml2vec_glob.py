    ...
    def get_note_type(self, divisions):
        ...
        if self.time_mod:# time modificationがあれば
            ...
            # それ以外
                ...
                raise ValueError("Error! Cannot use tuplet except quarter-note triplet and eigths-note triplet")
        ...
        if rate >= 0.0625 and rate < 0.125: # 長さが64分音符以上32分音符未満
            ...
            raise ValueError("Error! Cannnot process the notes whose duration are %d on divisions=%d" % self.duration, divisions)
        ...
        for m in range(1, piece_info.measure_num + 1):
            ...
            while cur_time < next_m_time:
                ...
                # もし違かったら
                    ...
                    print "cur_time:%d, melody[%d].time:%d" % (cur_time, i, melody[i].time)                 
                    raise ValueError("Error! Note time Error\ncur_time:%d, melody[%d].time:%d" % (cur_time, i, melody[i].time) 
            # while ここまで
